...
{  /**Purpose:Finish creating new ,or update or deleate relationship between product and supplier
   ...
   **/
    ...
    public partial class frmPrductSupplier : Form
    {
        ...
        public string currentProdName;//current selected item's product name
        public string currentSupName;//current seleted item's supplier name
        //for add button
        ...
        public string currentDeleteProductName;//get deleting product name
        public int currentDeleteSupplierId;//get deleting product supplier ID
        public string currentDeleteSupplierName;//get deleting supplier name
        public int currentDeleteProductSupplierId;//get deleting productSupplierID
        ...
        public void Display() //display CurrentProductsSuppliers in bottom Grid
        {
        ...
        private void  refreshLstProducts()//refresh lstSupplier which lists all related suppliers for one specific product
        {
          //  Suppliers = SupplierDB.GetAllsuppliers();
            ...
            lstSuppliers.DataSource = relatedSupplierName.ToList();//set LIQN result to supplier listbox
        ...
        private void cklstProductSupplier_ItemCheck(object sender, ItemCheckEventArgs e)//refresh checklistbox when it loaded again
        {
        ...
        private void lstProducts_SelectedIndexChanged(object sender, EventArgs e)//refresh checklistbox when it loaded again
        {
         ...
         private void btnUpdate_Click(object sender, EventArgs e)
        {
             ...
             OperationSign = "Update";//when clikc update button ,then set "Update" value to oPERATIONsIGN
             //initialize form
            lblSave.Visible = true;
            btnSave.Enabled = true;
            cklstProductSupplier.Visible = true;
            ...
            currentPS = OperationPS.getCurrentPS(currentUpdateProductId, currentUpdatesupplierId, OperationSign);//GET selected ProdcutSupplierID
           // currentUpdateProductId = currentProdId;
            Display();
            UpdateAvailableSupplier();//call update oeration method
            btnUpdate.Enabled = false;
        ...
        private void txtProductSearch_KeyUp(object sender, KeyEventArgs e)//fuzzy seraching textbox
        {
            if (txtProductSearch.Text == "")//if the searching textbox is empty
            {
                Products = ProductDB.GetAllProducts();//call sql query statement for geting all products
            ...
            else//if you input any character in search texbox
            {

                Products = ProductDB.SearchAllProducts(txtProductSearch.Text);//call sql statement with Like %
                var allProductName = from pro in Products //get each row in sUppliers TABle
                                     orderby pro.ProdName
                                     select new { ProductId = pro.ProductID, ProductName = pro.ProdName };
                lstProducts.SelectedIndexChanged -= lstProducts_SelectedIndexChanged;
        ...
        private void btnCancel_Click(object sender, EventArgs e)//button for cancel
        {
        ...
        private void txtProductSearch_Click(object sender, EventArgs e)//initialize search textbox for product
        {
            txtProductSearch.Clear();
            txtProductSearch.ForeColor = Color.Black;
        }
        private void txtSupplierSearch_Click(object sender, EventArgs e)//initialize search textbox for supplier
        {
        ...
        {
            supSupplier.Form3 form3 = new supSupplier.Form3();
